    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="hidden md:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((metric, idx) => {
          const percent = Math.min((metric.current / metric.limit) * 100, 100);
          return (
            <Card key={idx} className="bg-background/60 border border-border backdrop-blur-md shadow-sm rounded-2xl transition-all duration-300 hover:shadow-md hover:border-accent/30 group">
              <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                <CardTitle className="text-sm font-semibold text-muted-foreground font-sans">
                  {metric.title}
                </CardTitle>
                <div className={`p-2 rounded-xl transition-transform group-hover:scale-110 duration-300 ${metric.color}`}>
                  <metric.icon className="w-4 h-4" />
                </div>
              </CardHeader>
